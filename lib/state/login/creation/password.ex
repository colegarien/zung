defmodule Zung.State.Login.Creation.Password do
  ...
  defp handle_password(%Zung.Client{} = client, data) do
    _ = Zung.Client.raw_write(client, "||ECHO_OFF||")
    password_action = prompt_password(client)
    _ = Zung.Client.raw_write(client, "||ECHO_ON||||NL||")

    case password_action do
      {:ok, password} ->
        {Zung.State.Login.Creation.ColorCheck, client,
         Map.put(data, :password, Zung.Client.User.hash_password(data[:username], password))}

      {:error, message} ->
        _ = Zung.Client.raw_write_line(client, message)
        handle_password(client, data)
    end
  end
  ...
  defp verify_with_reentry(client, password) do
    _ = Zung.Client.raw_write(client, "||NL||Please re-enter password: ")
    ...
    with data <- Zung.Client.raw_read(client),
         ...
         do:
           if(other_password === password,
             do: {:ok, password},
             else: {:error, "Entries do not match."}
           )
  end
