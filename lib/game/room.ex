...
defmodule Zung.Game.Room.Exit do
  ...
  def describe(%Zung.Game.Room.Exit{name: name, direction: direction, description: description}) do
    cond do
      ...
      direction !== nil -> "Nothing to see, just an exit #{direction_to_text(direction)}."
      true -> "Nothing to see, just an exit."
    end
  end
  ...
  def describe_target(exits, target) do
    ...
    if Enum.count(matching_exits) > 0 do
      ...
      cond do
        target in [:north, :south, :east, :west, :up, :down] -> "There is nothing of interest to see #{direction_to_text(target)}."
        true -> "There is nothing of interest to see."
      end
    end
  end
